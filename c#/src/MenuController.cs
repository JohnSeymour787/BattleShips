...
namespace BattleShips
{
	...
	static class MenuController
	{
		...
		private static void DrawButtons(int menu, int level, int xOffset)
		{
			...
			toDraw.X = TEXT_OFFSET + MENU_LEFT + (BUTTON_SEP * xOffset);
			toDraw.Y = TEXT_OFFSET + MENU_TOP - (MENU_GAP + BUTTON_HEIGHT) * level;
			...
			for (i = 0; i <= _menuStructure[menu].Length - 1; i++)
			{
				...
				int btnLeft = (int)toDraw.X - TEXT_OFFSET;
				int btnTop = (int)toDraw.Y - TEXT_OFFSET;

				if (SwinGame.MouseDown(MouseButton.LeftButton) &&
					IsMouseOverMenu(i, level, xOffset))
				{
					SwinGame.DrawRectangle(
						HIGHLIGHT_COLOR,
						btnLeft,
						btnTop,
						BUTTON_WIDTH,
